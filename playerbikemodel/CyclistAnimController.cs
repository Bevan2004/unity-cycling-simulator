...
namespace SBPScripts
{
    public class CyclistAnimController : MonoBehaviour
    {
        ...
        void Update()
        {
            // [!CHANGED] Use GetCurrentSpeed() instead of rb.linearVelocity
            speed = bicycleController.GetCurrentSpeed();
            
            isAirborne = bicycleController.isAirborne;
            anim.SetFloat("Speed", speed);
            anim.SetBool("isAirborne", isAirborne);

            // ... (Rest of the script remains exactly the same as original)
            if (cyclist != null && externalCharacter != null)
            {
                if (Input.GetKeyDown(KeyCode.Return) && speed <= 0.1f && waitTime == 0)
                {
                    waitTime = 1.5f;
